const { ccclass, property, requireComponent, executionOrder, disallowMultiple, help } = cc._decorator;

const Enum = cc.Enum
const Node = cc.Node; type Node = cc.Node
const Prefab = cc.Prefab; type Prefab = cc.Prefab
const ValueType = cc.ValueType; type ValueType = cc.ValueType
...
class _scroll_view extends ScrollView {
    ...
    protected hasNestedViewGroup(event: Event, captureListeners?: Node[]): boolean {
...
export class YXCollectionView extends Component {
     ...
     * 根据当前的可见区域调整需要显示的节点
     */
    private reloadVisibleCells(rect: cc.Rect = null) {
        // 获取当前可见区域
        if (rect == null) {
    ...
    protected onLoad(): void {
        for (let index = 0; index < this.registerCellForEditor.length; index++) {
            ...
            this.register(element.identifier, () => cc.instantiate(element.prefab), element.comp)
        }
