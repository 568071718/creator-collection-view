const { ccclass, property, executionOrder, disallowMultiple, help } = cc._decorator;

const Enum = cc.Enum
const Node = cc.Node; type Node = cc.Node
const Prefab = cc.Prefab; type Prefab = cc.Prefab
const NodePool = cc.NodePool; type NodePool = cc.NodePool
const Component = cc.Component; type Component = cc.Component
const ScrollView = cc.ScrollView; type ScrollView = cc.ScrollView
const EventMouse = cc.Event.EventMouse; type EventMouse = cc.Event.EventMouse
const EventTouch = cc.Event.EventTouch; type EventTouch = cc.Event.EventTouch
const Mask = cc.Mask; type Mask = cc.Mask
const NodeEventType = cc.Node.EventType; type NodeEventType = cc.Node.EventType

const _vec3Out = new cc.Vec3()
const _scroll_view_visible_rect = new cc.Rect()
const _recycleInvisibleNodes_realFrame = new cc.Rect()
...
class _scroll_view extends ScrollView {
    protected _yx_scroll_offset_on_touch_start: cc.Vec2 = null
    _yx_startAttenuatingAutoScrollTargetOffset: (touchMoveVelocity: cc.Vec3, startOffset: cc.Vec2, originTargetOffset: cc.Vec2, originScrollTime: number) => { offset: cc.Vec2; time?: number; attenuated?: boolean; } = null
     ...
     */
    protected _startAttenuatingAutoScroll(deltaMove, initialVelocity) {
    ...
    protected hasNestedViewGroup(): boolean {
...
export class YXLayoutAttributes {
     ...
     */
    get frame(): cc.Rect { return this._frame }
    private _frame: cc.Rect = new cc.Rect()
...
export abstract class YXLayout {
     ...
     */
    targetOffset(collectionView: YXCollectionView, touchMoveVelocity: cc.Vec3, startOffset: cc.Vec2, originTargetOffset: cc.Vec2, originScrollDuration: number): { offset: cc.Vec2; time?: number; attenuated?: boolean; } | null { return null }
    ...
    layoutAttributesForSupplementaryAtIndexPath(indexPath: YXIndexPath, kinds: string): YXLayoutAttributes {
     ...
     */
    scrollTo(indexPath: YXIndexPath, collectionView: YXCollectionView): cc.Vec2 { return null }
...
export class YXCollectionView extends Component {
    ...
    private _reloadData() {
        ...
        if (this.mode == _yx_collection_view_list_mode.PRELOAD) {
            ...
            // 销毁所有预加载的节点
            this.preloadNodesMap.forEach((value) => {
                if (value) {
                    value.removeFromParent()
                    value.destroy()
                }
     ...
     * 根据当前的可见区域调整需要显示的节点
     */
    private reloadVisibleElements(visibleRect: cc.Rect = null) {
    ...
    private recycleInvisibleNodes(visibleRect: cc.Rect = null) {
        ...
        this.visibleNodesMap.forEach((value, key) => {
             ...
             */
            let boundingBox = value.getBoundingBox()
    ...
    private applyLayoutAttributes(node: Node, attributes: YXLayoutAttributes) {
        ...
        if (attributes.offset) {
            cc.Vec3.add(_vec3Out, _vec3Out, attributes.offset)
        }
    ...
    protected onLoad(): void {
        for (let index = 0; index < this.registerCellForEditor.length; index++) {
            ...
            this.registerCell(element.identifier, () => cc.instantiate(element.prefab), element.comp)
        }
        for (let index = 0; index < this.registerSupplementaryForEditor.length; index++) {
            ...
            this.registerSupplementary(element.identifier, () => cc.instantiate(element.prefab), element.comp)
        }
